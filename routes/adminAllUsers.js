...
router.get('/', async function(req, res, next) {
    ...
    try
    {
        const result = await axios.get('http://localhost:5000/api/users/search/' + 'admin@nitc.ac.in');
...
router.post('/', async function(req, res, next) {
    ...
    console.log(body);
    try{
        const reportResult = await axios.get('http://localhost:5000/api/reports/getAllReports');
        const result = await axios.get('http://localhost:5000/api/users/search/' + 'admin@nitc.ac.in');
        const allUsers = await axios.post('http://localhost:5000/api/users/searchUser', body);
        const perDayData = await axios.post('http://localhost:5000/api/reports/getPerDayReports', {date:today});
